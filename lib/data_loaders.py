...
class IndoorPairDataset(PairDataset):
  ...
  def __getitem__(self, idx):
    ...
    if(not os.path.exists(image_file0)):
      image_file0 = file0.replace(".ply","_0.jpg")
    if(not os.path.exists(image_file1)):
      image_file1 = file1.replace(".ply","_0.jpg")

    p_pc = o3d.io.read_point_cloud(file0)
    ...
    # Voxelization
    x1 = ME.utils.sparse_quantize(xyz0 / self.voxel_size, return_index=True)
    x2 = ME.utils.sparse_quantize(xyz1 / self.voxel_size, return_index=True)
    ...
    normals0 = np.asarray(p_pc.normals)
    normals1 = np.asarray(q_pc.normals)
    if(len(normals0) > 0):
      pcd0.normals = o3d.utility.Vector3dVector(normals0[sel0])
    if(len(normals1) > 0):
      pcd1.normals = o3d.utility.Vector3dVector(normals1[sel1])
...
class KITTIPairDataset(PairDataset):
  AUGMENT = None
  DATA_FILES = {
      'train': '/data1/zhangliyuan/code/IMFNet/config/train_kitti.txt',
      'val': '/data1/zhangliyuan/code/IMFNet/config/val_kitti.txt',
      'test': '/data1/zhangliyuan/code/IMFNet/config/test_kitti.txt'
  }
  ...
  def __getitem__(self, idx):
    ...
    return (
      unique_xyz0_th.float(),
      unique_xyz1_th.float(),
      coords0.int(),
      coords1.int(),
      feats0.float(),
      feats1.float(),
