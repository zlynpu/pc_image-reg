...
ROOT_DIR = osp.abspath('/data1/zhangliyuan/code/IMFNet')
...
def read_rgbd_image(cfg, color_file, depth_file, convert_rgb_to_intensity):
    ...
    rgbd_image = o3d.geometry.PointCloud.create_rgbd_image_from_color_and_depth(
...
def process_single_fragment(cfg, color_files, depth_files, frag_id, n_frags, intrinsic_path,
    ...
    volume = o3d.geometry.integration.ScalableTSDFVolume(
        voxel_length=cfg.tsdf_cubic_size / 512.0,
        sdf_trunc=0.04,
        color_type=o3d.geometry.integration.TSDFVolumeColorType.RGB8)
...
def run_seq(cfg, scene, seq):
    ...
    color_names = uio.list_files(seq_folder, '*.color.jpg', alphanum_sort=True)
...
def parse_args():
    ...
    root_path = "/DISK/qwt/datasets/an2"
    out_root = "../../../3DMatch_fusion_analysis"
    parser.add_argument('--dataset_root', default=root_path)
