...
ROOT_DIR = osp.abspath('/data1/zhangliyuan/mycode/AAAI_2023')
...
def register_fragment_pair(
    ...
    fi = osp.join(desc_root,scene_name,seq_name, frag1_name + ".ply")
    data_i = o3d.io.read_point_cloud(fi)
    image_file_i = fi.replace(".ply", "_0.png")
    if (not os.path.exists(image_file_i)):
          image_file_i = fi.replace(".ply", "_0.jpg")
          suffix = ".jpg"
    pc_image_i = image.imread(image_file_i)

    if (pc_image_i.shape[0] != 120 or pc_image_i.shape[1] != 160):
        pc_image_i = process_image(image=pc_image_i, aim_H=120, aim_W=160)
    pc_image_i = np.transpose(pc_image_i, axes=(2, 0, 1))
    pc_image_i = np.expand_dims(pc_image_i,axis=0)

    fj = osp.join(desc_root,scene_name,seq_name, frag1_name + ".ply")
    data_j = o3d.io.read_point_cloud(fj)
    image_file_j = fj.replace(".ply", "_0.png")
    if (not os.path.exists(image_file_j)):
          image_file_j = fj.replace(".ply", "_0.jpg")
          suffix = ".jpg"
    pc_image_j = image.imread(image_file_j)

    if (pc_image_j.shape[0] != 120 or pc_image_j.shape[1] != 160):
        pc_image_j = process_image(image=pc_image_j, aim_H=120, aim_W=160)
    pc_image_j = np.transpose(pc_image_j, axes=(2, 0, 1))
    pc_image_j = np.expand_dims(pc_image_j,axis=0)

    with torch.no_grad():
        coord_i, feat_src, coord_j, feat_tgt = extract_features(
          model,
          src_xyz=np.array(data_i.points),
          tgt_xyz=np.array(data_j.points),
          rgb=None,
          normal=None,
          voxel_size=voxel_size,
          device=device,
          skip_check=True,
          src_image=pc_image_i,
          tgt_image=pc_image_j)
    
    feat_i = feat_src.detach().cpu().numpy()
    feat_j = feat_tgt.detach().cpu().numpy()
    points_i = np.array(data_i.points)
    points_j = np.array(data_j.points)
...
def run_scene_matching(device,model,scene_name,                  # current scene
...
def parse_args():
    ...
    out_path = "/data1/zhangliyuan/mycode/AAAI_2023/result/3dmatch/exp1"

    desc_path = "/data1/zhangliyuan/code/IMFNet_exp/dataset/3DImageMatch/3DImageMatch/3DMatch_test"
    desc_type = "IMFNet"

    model_path = '/data1/zhangliyuan/mycode/AAAI_2023/outputs/3dmatch_raw/checkpoint_epoch_20_feat_match_ratio_0.9825.pth'
